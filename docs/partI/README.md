...
| 資料表         | 主鍵名稱 | 資料型態                     | 說明                             | 值域       |
|----------------|----------|------------------------------|----------------------------------|------------|
| **User**       | userId   | `INT(11) UNSIGNED` (4 bytes) | 使用者的唯一識別碼               | 僅限正整數 |
| **Book**       | bookId   | `INT(11) UNSIGNED` (4 bytes) | 書籍（正本）的唯一識別碼         | 僅限正整數 |
| **BookTitle**  | titleId  | `INT(11) UNSIGNED` (4 bytes) | 書籍標題（不同語言）的唯一識別碼 | 僅限正整數 |
| **AccessCopy** | copyId   | `INT(11) UNSIGNED` (4 bytes) | 借閱紀錄的唯一識別碼             | 僅限正整數 |
...
- Book
  ...
  | 欄位名稱 | 欄位說明 | 資料型態    | 值域                  | 是否為空 |
  | -------- | -------- | ----------- | --------------------- | ---------- |
  | isbn     | 書籍ISBN | varchar(13) | 必須符合 ISBN-13 格式 | 否         |
